...
def run(context):
    ...
    # Configuration
    model_id = config.get("sd_model", "SG161222/Realistic_Vision_V5.1_noVAE")
    use_ip_adapter = config.get("use_ip_adapter", True)
    ...
    if use_ip_adapter:
        ...
        # realistic vision is SD 1.5 based
    ...
    seed = config.get("flux_seed", 42)
    generator = torch.Generator(device=device).manual_seed(seed)
    
    reference_image = None
    ref_image_path = config.get("reference_image_path", None)
    
    if use_ip_adapter and ref_image_path and os.path.exists(ref_image_path):
    ...
    for i, scene in enumerate(script_data["scenes"]):
        ...
        kwargs = {
            "prompt": prompt,
            "negative_prompt": "blurry, text, watermark, bad anatomy, deformed",
            "num_inference_steps": config.get("sd_steps", 20),
            "guidance_scale": config.get("sd_guidance", 7.5),
            "generator": generator,
            "width": 768,
            "height": 1360
        }
        ...
        if use_ip_adapter and reference_image is None:
